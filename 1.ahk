...
SendDiscordText() {
    ...
    webhookUrl := "https://discord.com/api/webhooks/1322450690555248754/56snsciH2Wa7ppamg_AV7xl2UJO5GJEgLdaZTWPdhTs6uCuDcqzNBvlIotqqlz-A3XmV"
    
    textContent := "Found God Pack in Instance #" . scriptName . "!"
    RunWait, curl -X POST %webhookUrl% -H "Content-Type: application/json" -d "{\"content\": \"%textContent%\"}",, Hide
}

SendDiscordImage() {
    global scriptName, screenshotTime  ; 使用全局時間戳
    
    webhookUrl := "https://discord.com/api/webhooks/1322450690555248754/56snsciH2Wa7ppamg_AV7xl2UJO5GJEgLdaZTWPdhTs6uCuDcqzNBvlIotqqlz-A3XmV"
    
    screenshotPath := A_ScriptDir . "\..\Screenshots\" . screenshotTime . ".png"  ; 使用相同的時間戳
    
    if (FileExist(screenshotPath)) {
        RunWait, curl -F "file=@%screenshotPath%" %webhookUrl%,, Hide
    }
}
...
Screenshot() {
	global adbShell, adbPath, screenshotTime
	SetWorkingDir %A_ScriptDir%  ; Ensures the working directory is the script's directory
	
	screenshotTime := A_Now
	...
	screenshotFile := screenshotsDir "\" screenshotTime ".png"
